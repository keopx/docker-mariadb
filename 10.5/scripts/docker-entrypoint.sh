...
docker_verify_minimum_env() {
	if [ -z "$DB_ROOT_PASSWORD" -a -z "$DB_ALLOW_EMPTY_PASSWORD" -a -z "$DB_RANDOM_ROOT_PASSWORD" ]; then
		mysql_error $'Database is uninitialized and password option is not specified\n\tYou need to specify one of DB_ROOT_PASSWORD, DB_ALLOW_EMPTY_PASSWORD and DB_RANDOM_ROOT_PASSWORD'
	fi
}
...
docker_setup_db() {
	...
	if [ -z "$DB_INITDB_SKIP_TZINFO" ]; then
		# sed is for https://bugs.mysql.com/bug.php?id=20545
		mysql_tzinfo_to_sql /usr/share/zoneinfo \
			| sed 's/Local time zone must be set--see zic manual page/FCTY/' \
			| docker_process_sql --dont-use-mysql-root-password --database=mysql
			# tell docker_process_sql to not use DB_ROOT_PASSWORD since it is not set yet
	fi
	# Generate random root password
	if [ -n "$DB_RANDOM_ROOT_PASSWORD" ]; then
		export DB_ROOT_PASSWORD="$(pwgen -1 32)"
		mysql_note "GENERATED ROOT PASSWORD: $DB_ROOT_PASSWORD"
	fi
	# Sets root password and creates root users for non-localhost hosts
	local rootCreate=
	# default root to listen for connections from anywhere
	if [ -n "$DB_ROOT_HOST" ] && [ "$DB_ROOT_HOST" != 'localhost' ]; then
		# no, we don't care if read finds a terminating character in this heredoc
		# https://unix.stackexchange.com/questions/265149/why-is-set-o-errexit-breaking-this-read-heredoc-expression/265151#265151
		read -r -d '' rootCreate <<-EOSQL || true
			CREATE USER 'root'@'${DB_ROOT_HOST}' IDENTIFIED BY '${DB_ROOT_PASSWORD}' ;
			GRANT ALL ON *.* TO 'root'@'${DB_ROOT_HOST}' WITH GRANT OPTION ;
		EOSQL
	fi

	# tell docker_process_sql to not use DB_ROOT_PASSWORD since it is just now being set
...
_mysql_passfile() {
	...
	if [ '--dont-use-mysql-root-password' != "$1" ] && [ -n "$DB_ROOT_PASSWORD" ]; then
		cat <<-EOF
			[client]
